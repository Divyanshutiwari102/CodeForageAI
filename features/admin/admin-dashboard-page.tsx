          ...
          return (
            <div key={labels[idx]}>
              ...
              <div className="h-2 w-full rounded bg-white/[0.06]">
                <div className="h-2 rounded bg-violet-400" style={{ width }} />
              </div>
            </div>
          );
        })}
    ...
    <DashboardShell>
      ...
      <main className="space-y-6 p-4 sm:p-6">
        <section>
          ...
          <p className="text-sm text-white/60">Operational counters, Redis reliability, and payment verification health (live refresh every 10s).</p>
          {metrics ? (
            <p className="mt-2 text-xs text-white/60">Last updated: {new Date(metrics.timestamp).toLocaleString()}</p>
          ) : null}
        </section>
        ...
        <section className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          {loading || !metrics ? (
            ...
            <>
              ...
              <Card className="p-4">
                <p className="text-xs text-white/60">Failure Alert Count</p>
                <p className="mt-2 text-xl font-semibold text-white">{metrics.paymentHighFailureRateAlertCount}</p>
                <p className="mt-1 text-xs text-white/60">Threshold: {metrics.paymentHighFailureRateThresholdPercent}%</p>
              </Card>
            </>
          )}
        </section>
        ...
        <section className="space-y-2">
          ...
          <p className="text-sm text-white/60">Trace-aware request audit logs for debugging across services.</p>
          <AuditLogViewer />
        </section>
        ...
        <section className="grid gap-3 sm:grid-cols-3">
          {loading || !metrics ? (
            ...
            <>
              <Card className="p-4">
                <p className="text-xs text-white/60">Redis Circuit State</p>
              ...
              <Card className="p-4">
                <p className="text-xs text-white/60">Circuit Open Until</p>
                <p className="mt-2 text-sm font-semibold text-white">{circuitOpenUntil}</p>
              </Card>
              <Card className="p-4">
                <p className="text-xs text-white/60">Monitoring Visibility</p>
